...
public class TaulerSudoku extends Tauler {
    ...
    public TaulerSudoku(int n) throws ExcepcionTamanoIncorrecto, ExcepcionPosicionFueraRango, ExcepcionNumCeldasDiferenteTamano { 
        super(n*n, n*n);
        this.n = n;
        rellenaRegiones();
    }
   ...
   private void rellenaRegiones() throws ExcepcionPosicionFueraRango, ExcepcionTamanoIncorrecto, ExcepcionNumCeldasDiferenteTamano { //MEJORABLE POR EL FOR DE 3 Y LUEGO SWITCH
    ...
    public boolean esPosible(int x, int y, int val) throws ExcepcionValorFueraRango, ExcepcionPosicionFueraRango{
    	if (x < 0 || x > n*n || y < 0 || y > n*n) throw (new ExcepcionPosicionFueraRango());
		if (val < 0 || val > n*n) throw (new ExcepcionValorFueraRango());
    ...
    private void fijar(int x, int y) throws ExcepcionCasillaVaciaNoFijable, ExcepcionPosicionFueraRango {
    	if (x < 0 || x > n*n || y < 0 || y > n*n) throw (new ExcepcionPosicionFueraRango());
		int posRegFila = x;
		rs[posRegFila].getCella(y).fijar(); //Con hacerlo para una region ya vale porque se hace en la fila realmente
    }
    
    private void SetNumEnFila(int x, int y, int val) throws ExcepcionValorYaPuesto, ExcepcionNumeroFijo, ExcepcionPosicionFueraRango, ExcepcionValorFueraRango {
    	if (x < 0 || x > n*n || y < 0 || y > n*n) throw (new ExcepcionPosicionFueraRango());
		if (val < 0 || val > n*n) throw (new ExcepcionValorFueraRango());
    ...
    private void SetNumEnColumna(int x, int y, int val) throws ExcepcionValorYaPuesto, ExcepcionPosicionFueraRango, ExcepcionValorFueraRango, ExcepcionNumeroFijo {
    	if (x < 0 || x > n*n || y < 0 || y > n*n) throw (new ExcepcionPosicionFueraRango());
		if (val < 0 || val > n*n) throw (new ExcepcionValorFueraRango());
		int posRegColumna = y + (n*n);
    ...
    private void SetNumEnCuadrado(int x, int y, int val) throws ExcepcionValorYaPuesto, ExcepcionPosicionFueraRango, ExcepcionValorFueraRango, ExcepcionNumeroFijo {
    	if (x < 0 || x > n*n || y < 0 || y > n*n) throw (new ExcepcionPosicionFueraRango());
		if (val < 0 || val > n*n) throw (new ExcepcionValorFueraRango());
		//CALCULA POSICION:
		//posicion de la region cuadrado en el vector rs: (fmc*n + cmc) + (n*n * 2)
    ...
    private void borraNumEnFila(int x, int y) throws ExcepcionPosicionFueraRango, ExcepcionNumeroFijo {
    	if (x < 0 || x > n*n || y < 0 || y > n*n) throw (new ExcepcionPosicionFueraRango());
		int posRegFila = x; //posicion de la region fila en el vector rs: x + 0
		rs[posRegFila].borra(y);
    }
    
    private void borraNumEnColumna(int x, int y) throws ExcepcionPosicionFueraRango, ExcepcionNumeroFijo {
    	if (x < 0 || x > n*n || y < 0 || y > n*n) throw (new ExcepcionPosicionFueraRango());
		int posRegColumna = y + (n*n); //posicion de la region columna en el vector rs: y + (n*n)
        rs[posRegColumna].borra(x);
    }
    
    private void borraNumEnCuadrado(int x, int y) throws ExcepcionPosicionFueraRango, ExcepcionNumeroFijo {
    	if (x < 0 || x > n*n || y < 0 || y > n*n) throw (new ExcepcionPosicionFueraRango());
		//CALCULA POSICION:
		//posicion de la region cuadrado en el vector rs: (fmc*n + cmc) + (n*n * 2)
		//numero de fila de region cuadrado (no de la celda): fmc = x/n (es x/n truncado)
	...
	public void borraNumCelda(int x, int y) throws ExcepcionPosicionFueraRango, ExcepcionNumeroFijo, ExcepcionCasillaBloqueada, ExcepcionValorFueraRango {
		if (x < 0 || x > n*n || y < 0 || y > n*n) throw (new ExcepcionPosicionFueraRango());
	...
	private void numeroYaNoEstaEnRegiones(int x, int y) throws ExcepcionValorFueraRango, ExcepcionPosicionFueraRango {
		if (x < 0 || x > n*n || y < 0 || y > n*n) throw (new ExcepcionPosicionFueraRango());
		//CALCULA POSICIONES:
		//posicion de la region fila en el vector rs: x + 0
		int posRegFila = x + 0;
